    ...
    def build_observation(self, repre="feature"):
        ...
        if repre == "feature":
            ...
            for i, job in enumerate(self.job_queue):
                ...
                green_str = f"High green energy match, up to {green_val * 100}%" if green_state > 0.5 else f"Relies on brown energy, green usage limited to {green_val * 100}%"
                
                job_desc = (
                    f"Job {i}: Waited {wait_time} seconds. "
                    f"Requires {job.request_number_of_processors} cores and {job.power:.1f} Power "
                    f"for {job.request_time} seconds. "
